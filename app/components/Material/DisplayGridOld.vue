...
const cards = computed(() => {
  ...
  return uniqueCategories.map((category) => {
    return {
      ...
      // slot: `${category.title.toLowerCase().replace(/\s+/g, '-')}` as const
    }
  })
})
</script>
...
<template>
  <UStepper
    ...
    class="w-full"
    size="xl"
    :ui="{ title: 'flex flex-col gap-y-2 items-center justify-center text-base text-pretty font-semibold text-highlighted',
           trigger: 'p-1 rounded-sm group-data-[state=completed]:bg-primary/70 group-data-[state=active]:bg-primary/70',
           description: 'flex flex-col items-center justify-center',
           separator: 'group-data-[state=completed]:bg-primary/70'
    }"
  >
    <template #indicator="{ item }">
      ...
      <UIcon v-else :name="item.slotContent.icon" />
    </template>
    <template #title="{ item }">
