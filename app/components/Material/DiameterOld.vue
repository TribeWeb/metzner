...
const coordinates = computed(() =>
  calculateCircleCoordinates(slider.value, machineMax.value, width.value, height.value)
)

onMounted(() => {
  nextTick(() => {
    slider.value = diameter.value || machineMin.value
  })
})
...
<template>
  <div class="sm:w-96 flex flex-col">
    <div
      ref="el"
      class="p-3 bg-primary/10 border border-primary/50 rounded-t-lg relative inline-flex items-center justify-start"
    >
    ...
    <div class="px-3 pb-6 pt-14 border border-t-0 border-muted rounded-b-lg">
      <USlider
        ...
        :tooltip="{ text: `⌀${slider} mm`,
                    open: true,
                    arrow: true,
                    content: { side: 'top', avoidCollisions: true, collisionBoundary: [el], positionStrategy: 'absolute' } }"
        :min="1"
        :max="machineMax"
