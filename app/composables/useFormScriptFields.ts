...
export async function useFormScriptFields(formId: number, prospectFormOrgId: string) {
  const { data } = await useFetch<string>(`https://userresources.prospect365.com/forms/${prospectFormOrgId}/${formId}/form.js`)
  const htmlString = computed(() => data.value?.toString().split('.innerHTML=')?.[1]?.split('.className')[0])
